<x-app-layout pageName="{{ __('Edit User') }}">
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                   <h4 class="card-title"> <i class="feather icon-user-plus"></i> {{ __('Edit User #'.$user->id) }}</h4>
                </div>
                <div class="card-body">
                    <form action="{{ route('users.putupdate', $user->id) }}" method="post" class="needs-validation @if($errors->any()) was-validated @endif" novalidate>
                        @csrf
                        @method("put")
                        {{--  Basic Information  --}}
                        ...
                        <div class="form-row mb-3">
                            ...
                            <div class="form-group col-md-4 fill">
                                ...
                                <input type="text" name="middle_name" id="middle_name" value="{{ old('middle_name', $user->middle_name) }}" class="form-control">
                            </div>
                            <div class="form-group col-md-4 fill">
                        ...
                        <div class="form-row mb-3">
                            <div class="form-group col-md-6 fill">
                                ...
                                <input type="text" name="email" id="email" class="form-control" autocomplete="email" value="{{ old('email', $user->email) }}" required>
                            </div>
                            <div class="form-group col-md-6 fill">
                                <label for="contact_number" class="floating-label">Contact Number<x-form.optional-field/></label>
                                <input type="text" name="contact_number" id="contact_number" class="form-control" value="{{ old('contact_number', $user->contact_number) }}" autocomplete="mobile">
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="role" id="role" class="form-control" required>
                                ...
                                @foreach ($roles as $roleName => $role)
                                    <option value="{{ $roleName }}" {{ old('role', $user->role) == $roleName ? 'selected' : '' }}>{{ $role }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group text-center">
                            <button class="btn btn-outline-primary has-ripple">{{ __('Update User') }}</button>
                        </div>
                    </form>
                </div>
            </div>
